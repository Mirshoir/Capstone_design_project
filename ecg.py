...
def save_analysis_steps(stress_level, file_path):
    ...
    st.success(f"Stress level saved to `{file_path}`.")
...
def analyze_ecg_data(ecg_data, sampling_rate):
    ...
    if ecg_cal_std < 0.01:
        st.error("ECG_CAL_Voltage has very low variability. Check the data collection and sensor setup.")
        return
    ...
    if len(ecg_signal) < min_samples:
        st.error(f"Insufficient data length. Provide at least {MIN_DURATION} seconds of data ({min_samples} samples).")
        return
    ...
    try:
        ...
        st.error(f"Error during ECG cleaning: {e}")
        return
    ...
    try:
        ...
        st.error(f"Error during ECG peak detection: {e}")
        return
    ...
    if num_peaks < MIN_RPEAKS:
        st.warning(
            ...
            f"Please ensure that the ECG data is of sufficient length and quality."
        )
        return
    ...
    if num_peaks > 0:
        ...
        try:
            # Compute HRV metrics with adjusted parameters
            ecg_hrv = nk.hrv_time(ecg_peaks, sampling_rate=sampling_rate)
            if ecg_hrv.empty:
                st.warning("HRV DataFrame is empty. Unable to compute HRV metrics.")
                return

            st.subheader('ECG HRV Features')
            st.write(ecg_hrv)

            if 'HRV_SDNN' in ecg_hrv.columns and not ecg_hrv.empty:
                # Plot HRV_SDNN
                hrv_fig, hrv_ax = plt.subplots(figsize=(6, 4))
                hrv_ax.bar(['HRV_SDNN'], [ecg_hrv['HRV_SDNN'].iloc[0]], color='orange')
                hrv_ax.set_ylabel('SDNN (ms)')
                hrv_ax.set_title('HRV_SDNN')
                st.pyplot(hrv_fig)

                # Define population-based thresholds (hypothetical values)
                if ecg_hrv['HRV_SDNN'].iloc[0] < 50:
                    stress_level = 'High Stress'
                elif ecg_hrv['HRV_SDNN'].iloc[0] > 100:
                ...
                st.warning("No 'HRV_SDNN' feature found. Unable to estimate stress level.")
        except Exception as e:
            st.error(f"An error occurred while computing HRV from ECG: {e}")
    else:
        st.warning("No R-peaks detected in the ECG signal. Unable to compute HRV.")
...
def connect_shimmer(selected_port):
    ...
    try:
        ...
        print(f"Serial Error on port {selected_port}: {e}")
    except Exception as e:
        print(f"Could not connect on port {selected_port}: {e}")
    return None
...
def start_data_collection(selected_port, duration, csv_file_path):
    ...
    if shim_dev is None:
        st.error("Could not connect to the Shimmer device. Please check the COM port and device connection.")
        return None, None
